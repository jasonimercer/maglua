...
__global__ void getIPart(const int N_x, const int N_y, REAL* d_dest,  CUCOMPLEX* d_src)
{
	...
	d_dest[i + y * N_x] = d_src[i + y * N_x].y;
}
...
__global__ void twiddle2D(const int N_x, const int N_y, int* d_brp_x, int* d_brp_y, CUCOMPLEX* d_dest,  REAL* d_src)
{
	...
	d_dest[ti + ty * N_x].x = d_src[i + y * N_x];
	d_dest[ti + ty * N_x].y = 0;
}
...
// 	getIPart<<<blocks, threads>>>(N_x, N_y, p->d_output, p->d_GammaZZ[0]);
// 	CHECK
...
// 	timer = new_timer(); 
	...
	for(int k=0; k<3; k++) // x y z
	{
		for(int z=0; z<N_z; z++)
		{
			...
			const double* hs = &(sd[k].h_s[z*N_x*N_y]);
