...
// typedef struct s_fft_iteration
// {
// 	double W_angle;
// 	int destination;
// 	int source[1];
...
// static int sizeof_s_fft_iteration(int R)
// {
// 	return sizeof(double) * sizeof(int) * (R+1);
// }
...
// 	printf("max_depth %i\n", max_depth);
	for(int i=0; i<max_depth; i++)
	{
		...
		const int sz_d = dc * np * sizeof(double);
		
		plan->phase[i].h_src      = (int*   )malloc(sz_i);
		plan->phase[i].h_dest     = (int*   )malloc(sz_i);
		plan->phase[i].h_W_angles = (double*)malloc(sz_d);
		...
		double* wan = plan->phase[i].h_W_angles;
...
__global__ void Fourier_2D_x(const int nx, const int ny, 
			int* d_src, int* d_dest, double* d_W_angles,  
