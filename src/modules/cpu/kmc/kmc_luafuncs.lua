...
methods["apply"] = {
	...
	function(kmc, ss)
		...
		local ebaf_f = kmc:eventFunction() -- energy barrier, attempt frequenct
		...
		local min_time, min_action, min_pos
		local all_times = {}
		local function inner_loop(pos)
			...
			if eb then -- eb may be nil if in there is no other well to transition to
				...
				if min_time == nil or time < min_time then
					min_time = time
					min_action = action
					min_pos = pos
				end
				-- print(ss:time(), min_time, time)
				all_times[pos] = time
			end
		end
		...
		local mta = comm:gather(1, {min_time, min_action, min_pos}) -- send all proposed time/events to root
		local mta = comm:bcast(1, mta)

		-- find global min time
		min_time = mta[1][1]
		min_action = mta[1][2]
		min_pos = mta[1][3]
		for i=2,table.maxn(mta) do
			if mta[i][1] then 
				if min_time == nil or mta[i][1] < min_time then
					min_time = mta[i][1]
					min_action = mta[i][2]
					min_pos = mta[i][3]
				end
