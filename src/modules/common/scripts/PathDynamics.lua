...
function PathDynamics(mepOrig, json)
    ...
    -- Working on rate:
    local hessianAtSaddle = mep:hessianAtPoint(saddlePoint)

    local radial_terms = {}
    for i=1,ns do
	radial_terms[i] = (i-1)*3+1
    end
    local hessianAtSaddleNoRadial = hessianAtSaddle:cutX(radial_terms):cutY(radial_terms) -- cut radial terms

    local f12 = nil

    local hnr_evals, hnr_evecs = hessianAtSaddleNoRadial:matEigen()
    local evals, evecs = hessianAtSaddleNoRadial:matEigen()
    evecs = evecs:matTrans() -- so rows are vectors
    evals = evals:matTrans()
    ...
    local hessianAtMin = mep:hessianAtPoint(minimumEnergy1)
    local hessianAtMinNoRadial = hessianAtMin:cutX(radial_terms):cutY(radial_terms)
    local n1, n1_vecs = hessianAtMinNoRadial:matEigen() -- not actually using n1_vecs
    n1 = n1:matTrans()
    n1_vecs = n1_vecs:matTrans()
