...
bool Anisotropy::make_uncompressed()
{
	...
	if(new_host || !d_nx)
	{
		...
		malloc_device(&d_nx, sizeof(float)*nxyz);
		malloc_device(&d_ny, sizeof(float)*nxyz);
		malloc_device(&d_nz, sizeof(float)*nxyz);
		malloc_device(&d_k,  sizeof(float)*nxyz);
		
		
		memcpy_h2d(d_nx, h_nx, sizeof(float)*nxyz);
		memcpy_h2d(d_ny, h_ny, sizeof(float)*nxyz);
		memcpy_h2d(d_nz, h_nz, sizeof(float)*nxyz);
		memcpy_h2d(d_k,  h_k,  sizeof(float)*nxyz);
	}
	new_host = false;
	compressed = false;
	return true;
...
bool Anisotropy::make_compressed()
{
	...
	float* h_LUT;// = new float[unique * 4];
	unsigned char* h_idx;
	
	malloc_host(&h_LUT, sizeof(float) * unique * 4);
	...
	bool ok;
	ok  = malloc_device(&d_LUT, sizeof(float) * unique * 4);
	...
	memcpy_h2d(d_LUT, h_LUT, sizeof(float) * unique * 4);
...
const char* lib_name(lua_State* L)
{
	...
	return "Anisotropy-Cuda32";
#else
	return "Anisotropy-Cuda32-Debug";
#endif
}
