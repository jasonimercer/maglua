...
void SpinSystem::sync_spins_dh(bool force)
{
	if(new_device_spins || force)
	{
		...
		const size_t dxyz = sizeof(float) * nxyz;
...
void SpinSystem::diff(SpinSystem* other, double* v4)
{
	...
	ss_d_absDiffArrays32(d_ws1, d_x, other->d_x, nxyz);
	v4[0] = ss_reduce3DArray_sum32(d_ws1, d_ws2, h_ws1, nx, ny, nz);

	ss_d_absDiffArrays32(d_ws1, d_y, other->d_y, nxyz);
	v4[1] = ss_reduce3DArray_sum32(d_ws1, d_ws2, h_ws1, nx, ny, nz);

	ss_d_absDiffArrays32(d_ws1, d_z, other->d_z, nxyz);
	v4[2] = ss_reduce3DArray_sum32(d_ws1, d_ws2, h_ws1, nx, ny, nz);
...
bool SpinSystem::copySpinsFrom(lua_State* L, SpinSystem* src)
{
	...
	ss_d_copyArray32(d_x,  src->d_x,  nxyz);
	ss_d_copyArray32(d_y,  src->d_y,  nxyz);
	ss_d_copyArray32(d_z,  src->d_z,  nxyz);
	ss_d_copyArray32(d_ms, src->d_ms, nxyz);
...
bool SpinSystem::copyFieldFrom(lua_State* L, SpinSystem* src, int slot)
{
	...
	ss_d_copyArray32(d_hx[slot], src->d_hx[slot], nxyz);
	ss_d_copyArray32(d_hy[slot], src->d_hy[slot], nxyz);
	ss_d_copyArray32(d_hz[slot], src->d_hz[slot], nxyz);
...
void SpinSystem::init()
{
	...
	malloc_device(&d_x,  nx*ny*nz*sizeof(float));
	malloc_device(&d_y,  nx*ny*nz*sizeof(float));
	malloc_device(&d_z,  nx*ny*nz*sizeof(float));
	malloc_device(&d_ms, nx*ny*nz*sizeof(float));

	malloc_host(&h_x,  nx*ny*nz*sizeof(float));
	malloc_host(&h_y,  nx*ny*nz*sizeof(float));
	malloc_host(&h_z,  nx*ny*nz*sizeof(float));
	malloc_host(&h_ms, nx*ny*nz*sizeof(float));
	...
	ss_d_set3DArray32(d_x, nx, ny, nz, 0);
	ss_d_set3DArray32(d_y, nx, ny, nz, 0);
	ss_d_set3DArray32(d_z, nx, ny, nz, 0);
	ss_d_set3DArray32(d_ms, nx, ny, nz, 0);
	new_device_spins = true;
	sync_spins_dh();
	...
	{
		malloc_host(&h_ws1, sizeof(float)*min_size);
	}
	else
		malloc_host(&h_ws1, sizeof(float)*nx*ny*nz);
	
	
	d_hx = new float* [NSLOTS];
	d_hy = new float* [NSLOTS];
	d_hz = new float* [NSLOTS];
	
	h_hx = new float* [NSLOTS];
	h_hy = new float* [NSLOTS];
	h_hz = new float* [NSLOTS];
...
void SpinSystem::ensureSlotExists(int slot)
{
	...
	const int nn = sizeof(float) * nx*ny*nz;
	malloc_device(&(d_hx[slot]), nn);
	malloc_device(&(d_hy[slot]), nn);
	malloc_device(&(d_hz[slot]), nn);

	ss_d_set3DArray32(d_hx[slot], nx, ny, nz, 0);
	ss_d_set3DArray32(d_hy[slot], nx, ny, nz, 0);
	ss_d_set3DArray32(d_hz[slot], nx, ny, nz, 0);
...
void SpinSystem::sumFields()
{
	...
	for(int i=0; i<NSLOTS; i++)
	{
		...
		{
			sync_fields_hd(i);
			ss_d_add3DArray32(d_hx[SUM_SLOT], nx, ny, nz, d_hx[SUM_SLOT], d_hx[i]);
			ss_d_add3DArray32(d_hy[SUM_SLOT], nx, ny, nz, d_hy[SUM_SLOT], d_hy[i]);
			ss_d_add3DArray32(d_hz[SUM_SLOT], nx, ny, nz, d_hz[SUM_SLOT], d_hz[i]);
		}
	}
...
bool SpinSystem::addFields(double mult, SpinSystem* addThis)
{
	...
	ss_d_scaleadd3DArray32(d_hx[SUM_SLOT], nxyz, 1.0, d_hx[SUM_SLOT], mult, addThis->d_hx[SUM_SLOT]);
	ss_d_scaleadd3DArray32(d_hy[SUM_SLOT], nxyz, 1.0, d_hy[SUM_SLOT], mult, addThis->d_hy[SUM_SLOT]);
	ss_d_scaleadd3DArray32(d_hz[SUM_SLOT], nxyz, 1.0, d_hz[SUM_SLOT], mult, addThis->d_hz[SUM_SLOT]);
...
void SpinSystem::zeroField(int i)
{
	...
	ss_d_set3DArray32(d_hx[i], nx, ny, nz, 0);
	ss_d_set3DArray32(d_hy[i], nx, ny, nz, 0);
	ss_d_set3DArray32(d_hz[i], nx, ny, nz, 0);
...
void SpinSystem::getNetMag(double* v8)
{
	...
	float* d_ws1;// = (double*)getWSMem(sizeof(double)*nxyz);
	...
	sync_spins_hd();
	v8[0] = ss_reduce3DArray_sum32(d_x, d_ws1, h_ws1, nx, ny, nz);
	v8[1] = ss_reduce3DArray_sum32(d_y, d_ws1, h_ws1, nx, ny, nz);
	v8[2] = ss_reduce3DArray_sum32(d_z, d_ws1, h_ws1, nx, ny, nz);
...
void  getWSMem(float** ptr0, size_t size0, 
			   float** ptr1, size_t size1, 
			   float** ptr2, size_t size2,
			   float** ptr3, size_t size3,
			   float** ptr4, size_t size4)
{
	getWSMem( (void**) ptr0, size0, 
			  (void**) ptr1, size1, 
			  (void**) ptr2, size2, 
